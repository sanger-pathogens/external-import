...
class Preparation:
    ...
    def download_files_from_ena(self,connections):
        ...
        for i in range(connections, len(df)):
            ...
            df.loc[
                i, 'Command'] = 'bsub -o %s/%s.o -e %s/%s.e -J import_%s -w %s "/nfs/users/nfs_k/km22/external_import_development/enaBrowserTools/python3/enaDataGet -f fastq -d %s %s"' % (
            self.destination, df.loc[i, 'Read accession'], self.destination, df.loc[i, 'Read accession'], df.loc[i, 'Read accession'],
